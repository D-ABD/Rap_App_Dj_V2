...
class CommentaireViewSet(viewsets.ModelViewSet):
    ...
    def filter_options(self, request):
        ...
        """
        # ✅ import local (important pour éviter les cycles)
        ...
        # 🔒 Récupère uniquement les commentaires visibles selon le rôle utilisateur
        ...
        if is_admin_like(user):
            ...
            accessible_centre_ids = set(centre_ids).intersection(staff_centre_ids(user))
            centres = Centre.objects.filter(id__in=accessible_centre_ids)
        else:
            centres = Centre.objects.none()

        # --- Types d’offre réellement utilisés ---
        ...
        # --- Auteurs distincts (créateurs de commentaires) ---
        ...
        # --- Formations (uniquement celles liées à un commentaire) ---
    ...
    def get_queryset(self):
        ...
        # ------------------------------------------------------------------
        # Valeurs attendues : "actif" | "archive" | "all"
        statut_scope = (params.get("statut_id") or params.get("statut") or "").lower().strip()

        # Si c’est une action interne (retrieve, update, archiver, etc.), on ne restreint pas
        ...
        # ------------------------------------------------------------------
        # Par défaut : on cache les archivés
        if not statut_scope or statut_scope in {"actif", Commentaire.STATUT_ACTIF.lower()}:
            ...
            # fallback numérique : filtre sur statut de formation si applicable
            if statut_scope.isdigit():
    ...
    def list(self, request, *args, **kwargs):
        ...
        # 🔹 Permet au front de demander ?limit=1 (dernier commentaire)
        ...
        return Response(
            {"success": True, "message": "Commentaires récupérés", "data": serializer.data}
        )
    ...
    def create(self, request, *args, **kwargs):
        ...
        # ✅ Copie ici la saturation actuelle de la formation dans les données
        ...
        # ✅ Toujours renvoyer la version complète mise à jour
    ...
    @extend_schema(
        ...
        responses={
            200: OpenApiResponse(
                description="Données de saturation pour une formation", response=None
            )
        },
    )
    ...
    def meta(self, request):
        """
        Retourne les constantes utiles du modèle Commentaire :
        - bornes de saturation
        - longueur par défaut de preview
        - période par défaut des récents
        """
    ...
    def archiver(self, request, pk=None):
        instance = self.get_object()
        # Vérification déjà gérée par les permissions globales
